...
void ASStartMenu::Activate() {
	...
	m_menuOptions.Add(new MenuOption(EMO_LEVEL_1, String("Level 1")));
	m_menuOptions.Add(new MenuOption(EMO_LEVEL_2, String("Level 2")));
	m_menuOptions.Add(new MenuOption(EMO_LEVEL_3, String("Level 3")));
	m_menuOptions.Add(new MenuOption(EMO_EXIT_APP, String("Exit Game")));
...
void ASStartMenu::ProcessInput() {
	if (m_canInput == true) {
		if (Screen::Instance().KeyPressed(GLFW_KEY_ENTER)) {
			if (m_menuOptions[m_activeOption]->m_option == EMO_LEVEL_1) {
				GSetWantedState(EAS_GAME);
				g_currentMenuOp = EMO_LEVEL_1;
			} else if (m_menuOptions[m_activeOption]->m_option == EMO_LEVEL_2) {
				GSetWantedState(EAS_GAME);
				g_currentMenuOp = EMO_LEVEL_2;
			} else if (m_menuOptions[m_activeOption]->m_option == EMO_LEVEL_3) {
				GSetWantedState(EAS_GAME);
				g_currentMenuOp = EMO_LEVEL_3;
			} else if (m_menuOptions[m_activeOption]->m_option == EMO_EXIT_APP) {
				g_currentMenuOp = EMO_EXIT_APP;
				GSetWantedState(EAS_EXIT_APP);
			}
			RestartKeyElapsed();
		}
